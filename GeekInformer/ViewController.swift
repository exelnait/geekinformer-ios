...
class ViewController: UIViewController {
    ...
    @IBOutlet weak var myScrollViewbottom: UIScrollView!
...
//Создание экранов и их заполнение
    func createTrendsView(objects: [TrendsScreen]=[], ScrollView: UIScrollView){
        ...
        ScrollView.contentSize = CGSize(width: ScrollView.frame.width*CGFloat.init(objects.count/3), height: ScrollView.frame.height)
...
//Верхний скролл
        Alamofire.request("https://geekinformer.net/api/1/publishers/trends?type=resource").responseJSON()
            {
        response in
        if let value = response.result.value
        {
            ...
            self.createTrendsView(objects: objects, ScrollView: self.myScrollView)
        }
    }
    
//Нижний скролл
        Alamofire.request("https://geekinformer.net/api/1/publishers/trends?type=blogger").responseJSON()
            {
                response in
                if let value = response.result.value
                {
                    ...
                    self.createTrendsView(objects: objects, ScrollView: self.myScrollViewbottom)
                }
        }
